...
export const RTCProvider = (props: RTCProviderProps) => {
    ...
    useOnMount(() => {
        waitForElement(LEETCODE_SUBMIT_BUTTON, 2000)
            ...
            .catch((error) => {
                console.error(
                    "Error mounting callback on submit code button:",
                    error
                );
            });
    });

    const sendMessageToAll = React.useRef(
        (fn: ReturnType<typeof withPayload>) =>
    ...
    const joinRoom = React.useCallback(
        async (roomId: string): Promise<boolean> => {
            ...
            if (
                ...
                !roomQuestionId.includes(questionId)
            ) {
    ...
    const handleSucessfulSubmissionRef = React.useRef(
    ...
    React.useEffect(() => {
        if (roomId != null && getSnapshot()[roomId] == undefined) {
            const unsubscribe = onSnapshot(getRoomRef(roomId), (snapshot) => {
                ...
                // const questionId = getQuestionIdFromUrl(window.location.href);
                // const finishedUsers =
                //     data.questionMap[questionId]?.finishedUsers;
                // console.log(finishedUsers);
                // if (nextQuestion !== "") {
                //     toast.info(
                //         "The next chosen question is " +
                //             constructUrlFromQuestionId(nextQuestion)
                //     );
                // } else if (
                //     finishedUsers.every((user) => usernames.includes(user)) &&
                //     nextQuestion !== ""
                // ) {
                //     toast.info(
            ...
            if (message.data.action != undefined) {
                ...
                switch (windowMessage.action) {
                    case "leetCodeOnChange": {
                        getCodeMessagePayload(windowMessage.changes).then(
                            sendMessageToAll
                        );
                        break;
                    }
