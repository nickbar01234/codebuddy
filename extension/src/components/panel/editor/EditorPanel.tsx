...
import { useAppState, useOnMount, usePeerSelection } from "@cb/hooks/index";
...
const EditorPanel = () => {
  const { peers, activePeer, unblur, setActivePeerId, selectTest } =
    usePeerSelection();
  ...
  return (
    <div
      className={cn("flex flex-col relative h-full w-full", {
        hidden: emptyRoom || appState !== AppState.ROOM,
      })}
    >
